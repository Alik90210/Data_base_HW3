...
as
	...
	as
		(select first_name, last_name, customer_2.customer_id,
	...
	(
		...
		as
			(
				...
				select first_name, last_name, customer_id, trans_date, transaction_id,
				first_value(trans_date) over (partition by customer_id) as first_trunsacion_date,
				last_value(trans_date) over (partition by customer_id) as last_trunsaction_date
				from tansaction_date_customer_id
			)
			
		select first_name, last_name, customer_id, first_trunsacion_date, last_trunsaction_date,
		(last_trunsaction_date - first_trunsacion_date) as diff_day,
		
		rank() over (order by (last_trunsaction_date - first_trunsacion_date) desc) as rank_diff_day_desc
...
select first_name, last_name, customer_id, diff_day, rank_diff_day_desc
from final_rate
where rank_diff_day_desc = 1
